...
from retacred_utils import get_example_fields, get_label_id, precision_recall_fscore_exclude_nota, get_relation_set, \
...
def evaluate(model_family, client, model, tokenizer, retacred_file, n, experiment_name, open_source_api_mode=None):
    sleep = False
    save = False
    ...
    retacred_path = "../../data/retacred/"
    with open(retacred_path + retacred_file, "r") as f:
    ...
    for example in tqdm(examples, total=len(examples)):
        ...
        if predicted_label_id != label_id:
            relation_set_string = get_relation_set_string(relation_set)
            wrong_preds.append((example, predicted_label, relation_set_string))
